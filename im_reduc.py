...
for i in range(len(reduc_filters)):
    master_i_Flat[:, :, i] = draco.mast_reduc(file_path + 'Flats/' + reduc_filters[i] + '/', im_prefix + reduc_filters[i] + '-', FLAT_suffix, 10)
...
for n in  range(len(reduc_filters)):
    series_n[:, :, :, n] = draco.get_series(file_path + 'Lights/', c46P_prefix + reduc_filters[n] + '-', '', 5)
